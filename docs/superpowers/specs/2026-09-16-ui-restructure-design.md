...
### 분석 결과는 전체 화면 오버레이로 (2026-09-16 결정)

`[다시 정리]`는 남긴다. "거의 안 쓴다"는 "절대 안 쓴다"가 아니다.
`AnalysisResult` 타입과 `/api/analyze`, `onRegen` 경로는 **그대로 둔다.**

`NoteDocument`의 분석 블록 6개(핵심 요약 · 주요 논의 내용 · 결정사항 · 해야 할 일 ·
미결정사항 · 향후 일정, 약 350줄)를 `AnalysisView`로 옮겨 **전체 화면 오버레이**로
띄운다. 기본 화면에는 없고, `[다시 정리]`를 눌렀을 때와 이미 분석 결과가 있는 노트에서
`[분석 보기]`를 눌렀을 때만 열린다.

**오버레이를 고른 이유:** 블록 6개에는 `contentEditable` 편집, 할 일 추가 폼, 체크박스
토글이 붙어 있다. 오른쪽 패널(320px)에 밀어 넣으면 그 레이아웃을 전부 다시 짜야 하고,
표와 체크리스트가 좁은 폭에서 깨진다. 오버레이는 **블록을 그대로 옮기면 끝난다.**
가끔 여는 화면에 레이아웃 재작업 비용을 쓸 이유가 없다.

### 새 레이아웃

```
지금:  [트랜스크립트 380px] [핸들] [NoteDocument flex:1]
다음:  [LiveTranscript flex:1] [핸들] [오른쪽 패널 320px]
```

`review` 배너는 지금 `position:absolute`로 트랜스크립트 위에 겹쳐 있고, 본문이
`paddingTop: 44`로 그만큼을 비워 준다. 이 겹침을 없애고 **아래쪽 액션 바**로 옮긴다.

### 컴포넌트

| 새로 만드는 것 | 책임 | 어디서 왔나 |
|---|---|---|
| `AnalysisView` | 분석 블록 6개, 전체 화면 오버레이 | `NoteDocument` 블록 1~6 |
| `MeetingInfoPanel` | 회의 일시 · 장소 · 참석자 · 소요시간 + 자유 메모 | `NoteDocument`의 `PropRow` 3개 + 블록 7 |
| `NotionStatusPanel` | 동기화 상태, 페이지 링크, `[Notion 갱신]`, 건너뛴 속성, `[다시 정리]` | `AppShell`의 `bannerText` |
| `ActionBar` | `[이어 녹음] [Notion 갱신] [내보내기] [저장]` + 경과 시간 | `AppShell`의 review 배너 |

`NoteDocument`(818줄)는 삭제한다. 제목 `<h1>`은 되살리지 않는다 — 1단계에서 헤더 입력을
연결했으므로 **제목 입력구는 헤더 하나뿐**이 된다.

`새 발화 N개` 배지(`pendingTurnCount`)는 분석 블록에 붙어 있었다. 분석이 기본 화면에서
빠지므로 이 배지는 `AnalysisView`로 함께 간다 — "마지막 분석 이후 새 발화가 N개 있다"는
뜻이라 그 화면에서만 의미가 있다.

### Notion 페이지 링크

`NoteRecord.notionPageId`로 `https://notion.so/<하이픈 뺀 id>`를 만들어 연다.
`notionPageId`가 없으면 링크를 보여주지 않는다.
