...
### 🧠 2. 고품질 AI 분석 및 일관성
- **분석은 선택**: 회의 종료 시에는 분석하지 않고 Notion에 바로 저장합니다. 분석이 필요하면 **다시 정리**를 눌러 분석 화면을 열고, 그 안에서 실행하세요.
...
### 🖥 3. 트랜스크립트 중심 화면
- **가운데가 트랜스크립트**: 실제 결과물인 발화 기록이 화면 대부분을 차지합니다.
- **오른쪽 패널**: 회의 일시·장소·참석자·소요시간과 자유 메모, 그리고 Notion 동기화 상태와 페이지 링크. 핸들로 접을 수 있습니다.
- **분석은 오버레이**: **다시 정리**를 누르면 전체 화면으로 열립니다. `Esc`나 **닫기**로 빠져나옵니다.

### 📄 4. 섹션 최적화 PDF/Word 내보내기
