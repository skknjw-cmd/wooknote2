...
- **이어 녹음은 같은 페이지에**: 중지 후 다시 녹음하면 새 페이지를 만들지 않고, 이미 만든 Notion 페이지 끝에 `## 트랜스크립트 (이어 녹음)`으로 덧붙입니다. 앞부분은 건드리지 않으므로 그 사이에 Notion에서 손으로 정리한 내용이 남습니다.
- **제목은 바뀌었을 때만**: 이어 붙일 때 참석자·소요시간·상태는 늘 갱신하고, 제목은 앱에서 바꿨을 때만 보냅니다. 매번 덮어쓰면 Notion에서 고쳐 둔 제목이 되돌아가기 때문입니다.
