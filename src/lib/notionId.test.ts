...
describe("extractDatabaseId", () => {
  ...
  it("제목 슬러그가 날짜(2026-09-15-)로 시작해도 실제 ID를 뽑는다", () => {
    expect(
      extractDatabaseId("https://www.notion.so/ws/2026-09-15-a1b2c3d4e5f60718293a4b5c6d7e8f90"),
    ).toBe("a1b2c3d4e5f60718293a4b5c6d7e8f90");
  });

  it("제목 슬러그가 hex 문자(DB-)로 끝나도 실제 ID를 뽑는다", () => {
    expect(
      extractDatabaseId("https://www.notion.so/ws/DB-a1b2c3d4e5f60718293a4b5c6d7e8f90"),
    ).toBe("a1b2c3d4e5f60718293a4b5c6d7e8f90");
  });

  it("퍼센트 인코딩된 한글 제목(회의록)에서도 실제 ID를 뽑는다", () => {
    expect(
      extractDatabaseId(
        "https://www.notion.so/ws/%ED%9A%8C%EC%9D%98%EB%A1%9D-a1b2c3d4e5f60718293a4b5c6d7e8f90",
      ),
    ).toBe("a1b2c3d4e5f60718293a4b5c6d7e8f90");
  });

  it("제목 슬러그와 ?v= 뷰 ID가 함께 있어도 데이터베이스 ID를 뽑는다", () => {
    expect(
      extractDatabaseId(
        "https://www.notion.so/ws/2026-09-15-a1b2c3d4e5f60718293a4b5c6d7e8f90?v=0f1e2d3c4b5a69788796a5b4c3d2e1f0",
      ),
    ).toBe("a1b2c3d4e5f60718293a4b5c6d7e8f90");
  });
