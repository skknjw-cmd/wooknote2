...
**주요 논의 내용**
- 발언을 순서대로 나열하지 말 것. 이 회의의 핵심 논점만 추출해 항목별로 정리.
- 핵심요약과 동일한 구조: title은 논점 주제, description은 "[상황/배경] → [논의 흐름/결론]" 한 문장.
- 논의가 왜 일어났는지(배경), 어떤 입장 차이가 있었는지, 어떻게 마무리됐는지를 한 문장에 담을 것.
- title: 무엇에 대한 논의였는지 (주제어)
- description: "[배경] → [쟁점 또는 논의 결과]" 형식
  예) title: "Q2 공급 단가 조정", description: "원자재 가격 인상을 이유로 구매팀이 10% 인하 요구 → 공급사는 마진 한계로 5%까지만 가능하다는 입장으로 추가 협의 예정"
  예) title: "긴급 납기 단축 가능성", description: "신규 거래처 확보를 위해 14일 → 7일 단축 논의 → 공급사 생산 라인 확인 후 다음 주 결정"
