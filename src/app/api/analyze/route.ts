...
## 절대 원칙
1. JSON 구조·키 이름 변경 금지. 아래 구조를 그대로 사용할 것.
2. 섹션 name은 분석 요청 항목 문자열과 완전히 동일하게.
3. 내용 없는 섹션도 반드시 포함, content: [].
4. **줄글(산문) 절대 금지.** 모든 내용은 bullet 또는 표로만 작성.
5. 문체: 개조식(~함, ~임, ~예정). '합니다/입니다' 금지.
6. 구체적 수치·날짜·이름 명시. 추상적 서술 금지.
7. 불명확한 내용은 항목 끝에 [원문 확인 필요] 표기.

## 화자 처리
- "화자 1/2/3"을 문맥으로 역할명(구매팀, 개발팀, 공급사 측, 법무팀 등)으로 대체.
- 역할 파악 불가 시에만 "화자 N" 유지.
- "음", "어", "그러니까", "네 알겠습니다" 등 필러·맞장구 완전 무시.
- 중요도 순 정렬(시간 순 아님).
...
**핵심요약** (3~5개)
- title: 주제 키워드 (10자 이내)
- description: "현황 → 결과" 형식, 1문장 (예: "20% 증량 요청 → 15% 합의")
- 긴 설명 금지. 화살표(→) 형식 필수.

**주요 논의 내용**
- title: 논점 키워드 (10자 이내)
- description: 핵심 입장·쟁점만. 결론 아닌 논의 과정 기술. (예: "구매팀 10% 인하 요구 vs 공급사 5% 한계")
- 산문 서술 금지. 대립 구도는 "A vs B" 형식 활용.

**주요 결정사항**
- title: 결정 내용 (행동 주체 포함, 20자 이내)
- description: 조건·근거만 (없으면 빈 문자열)

**실행과제 (To-Do List)**
- task: 동사+목적어 형식 (예: "계약서 초안 작성")
- notes: 완료기준 필수 기재 (예: "초안 공유 시 완료"). 담당자·기한 불명확 시 "미정".

**미결정사항**
- title: 쟁점 키워드 (10자 이내)
- description: 현황 + 결정 필요 시점

**향후 일정**
- title: 날짜 또는 기한
- description: 일정 내용 (20자 이내)
