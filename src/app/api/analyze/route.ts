...
## 핵심 포맷 규칙
- 한 항목 = 하나의 완결된 사실. 여러 사실을 하나로 합치지 말 것.
- 각 bullet은 읽으면 내용을 이해할 수 있어야 함. 키워드만 나열하거나 지나치게 축약 금지.
- 단락(paragraph) 형식 금지. description은 항상 한 문장.
- **문어체 공문서 스타일 필수**: "홍길동 이사는 ~했다", "박대표는 ~라고 했다" 같은 구어체·서술체 금지.
  사람 이름·직책을 문장 주어로 쓰지 말 것. 대신 사안·행위·결과 중심으로 기술.
  ❌ "박교순 대표는 물량을 늘려달라고 요청했으며 노진욱 이사는 검토하겠다고 답함"
  ✅ "Q2 물량 20% 증량 요청 → 공급 가능 여부 검토 후 회신 예정"
...
- title: 주제명 (간결하게)
- description: [배경/현황] → [결과/방향] 형식의 한 문장. 사람 이름 주어 금지.
  예) title: "Q2 공급 물량 협상", description: "전년 대비 20% 증량 요청 → 15% 수용으로 합의"

**주요 논의 내용**
- 발언을 순서대로 나열하지 말 것. 핵심 논점만 추출해 항목별로 정리.
- title은 논점 주제, description은 "[상황/배경] → [논의 흐름/결론]" 한 문장.
- 논의 배경, 쟁점, 결론 흐름을 담되 사람 이름을 주어로 쓰지 말 것.
  입장 차이가 있을 경우 "요청 측 vs 공급 측" 등 역할로 표현.
- title: 무엇에 대한 논의였는지 (주제어)
- description: "[배경] → [쟁점 또는 논의 결과]" 형식
  예) title: "Q2 공급 단가 조정", description: "원자재 인상 근거로 10% 인하 요청 → 공급 측 마진 한계로 5%까지만 가능, 추가 협의 예정"
  예) title: "긴급 납기 단축", description: "신규 거래처 대응 목적으로 14일→7일 단축 논의 → 생산 라인 확인 후 다음 주 결정"
