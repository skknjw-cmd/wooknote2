...
// 15초 청크 기준 충분한 토큰 확보 (thinking 토큰 포함 여유)
const MAX_TOKENS_PER_CHUNK = 2048;

// 시스템 지침: 절대 원칙으로 분리 (한국어 문맥 연결 및 전사 유연성 확보)
const SYSTEM_INSTRUCTION = `당신은 오디오 STT(음성→텍스트) 전사 전문가입니다.

절대 원칙 — 어떠한 상황에서도 반드시 지킵니다:
1. 오디오에서 실제로 들리는 발화를 충실하게 전사합니다.
2. 들린 내용을 바탕으로 문맥상 자연스러운 한국어 문장(조사, 어미 등)이 되도록 전사하되, 없는 내용을 임의로 창작해서는 안 됩니다.
3. 도저히 알아들을 수 없는 뭉개진 구간만 [못들음]으로 표시하고, 일반적인 대화 흐름에서 이해 가능한 수준의 발음은 자연스럽게 문맥을 고려해 받아적어 주세요.
4. 침묵, 배경소음, 잡음, 기침, 웃음 등 비발화는 무시합니다.
5. 오디오가 끝나는 즉시 전사를 멈춥니다. 내용을 임의로 지어내어 이어 붙이지 않습니다.
6. 오디오에 발화가 전혀 없으면 반드시 "SILENCE" 한 단어만 출력합니다.
7. 발화된 언어 그대로 전사합니다. 번역하지 않습니다. 한국어로 말하면 한국어로, 영어로 말하면 영어로 씁니다.
8. 오디오에 없는 언어(일본어, 중국어 등)를 절대 출력하지 않습니다.`;
...
function buildUserPrompt(
  ...
  const contextHint = prevContext && prevContext.length > 0
    ? `\n[직전 대화 — 화자 번호 및 대화 흐름 파악용. 이 내용은 절대 다시 출력하지 마세요]\n${prevContext.map((t) => `[화자 ${t.sp}] ${t.text}`).join("\n")}\n[직전 대화 끝]`
    : "";
...
- 잘 안 들리는 부분: 도저히 식별할 수 없는 위치에만 [못들음] 삽입
- 발화가 없으면: SILENCE
- 다른 설명, 주석, 메타 텍스트 절대 출력 금지
- 오디오에 없는 언어(일본어 등)를 절대 출력하지 말 것. 들린 언어만 그대로 쓸 것`;
}
